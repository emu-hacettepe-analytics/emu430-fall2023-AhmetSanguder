...
title_names_1 <- html_text(title_names)
title_names_1 <- tail(head(title_names,-1),-1)
title_names_1 <- str_split(title_names, " ", n=2)
title_names_1 <- unlist(lapply(title_names, function(x) {x[2]}))
...
year2 <- as.numeric(html_text(year_2))
...
rating_2 <- html_text(rating2)
rating_2 <- as.numeric(str_extract(rating2, "\\d+\\.\\d+"))
